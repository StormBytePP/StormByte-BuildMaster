...
- [ ] **Idempotent stage stamps.** After the prefix is published, a later parent build that only changed the final project must not re-enter that leaf. `<id>_configure`, `<id>_build` and `<id>_install` stay current. `ninja <meta>_install` or any consumer of the same leaves (`ninja ffmpeg_install`) is a no-op for work already in the prefix. Today those nodes are custom targets with no `OUTPUT`, so every parent `make` treats them as dirty and re-enters nested `cmake --build` / `meson compile`. That re-entry is a rescan: it does not have to recompile anything, but it restats the nested build. The rescan is what re-reads an unpatched `CMakeLists.txt` (next item) and what will race a cache restore. Do not use the live worktree mtime as a stamp input. Post-install `RESET` dirties those mtimes on purpose, so a mtime stamp never stays current. Inputs are the submodule pin, the patch bytes, the normalized options, the toolchain profile, `CMAKE_BUILD_TYPE`, IPO on/off, `mode`, `produced` and host OS/arch — the same set as the install-cache key. The output is the published prefix. `NOINSTALL` stamps the builddir artefacts, not the prefix.
- [ ] **Re-apply `GIT={PATCH}` before any rebuild.** This is the body of the stamp command, not a second policy. The first `make -jN` is correct: PATCH runs before configure, the nested build sees the patched tree, then post-install `RESET` restores the submodule. A clean worktree is the invariant, and it removes the patches. The next parent `make` re-enters every BM leaf because the stamps are dirty. Nested `cmake --build` starts with `RERUN_CMAKE` / regenerate-during-build, parses `CMakeLists.txt` as it sits on disk (upstream, unpatched) and can die without rebuilding a single object. Canary: libsamplerate on CMake 4. The patch raises `cmake_minimum_required` to a legal value; upstream is illegal; the rescan fails on the minimum. Do not leave the worktree patched. Do not turn off regenerate-during-build — the failure is regenerating the wrong tree. When the stamp is dirty the command is PATCH, then the nested build, then install, then `RESET`. When the stamp is current nothing runs and the unpatched tree is never read. Compute the cache key from the pin plus the patch bytes; do not apply the patch to the live tree just to hash it.
...
- [ ] **Groups** can be created after they are used. `buildmaster_group_add` before `buildmaster_group` is legal, same rule as declaring a component after `buildmaster_link`. FATAL only at the end, and only if that group was never created. Cycles and id clashes stay FATAL. A group is still not a component, a meta or a link edge.
- [ ] **Shared-dep compile vs install race.** Declaring the same component more than once is first-wins, and that skip is the contract. A later `buildmaster_component` of an id that was already configured must not configure or build it again. Logger hosted by Buffer with `WITH_STORMBYTE=BUNDLED` logs `Skipping configure of StormByte Suite - String — already built by 'StormByte Suite - String' (StormByte-String)` and therefore never enters Base, because Base is declared only inside that skipped cmake. Correct. What the skip must not drop is the edge the later consumer still has. "Do not build twice" is not "this consumer has no dependency". Two obligations remain. Order: wait on the winner's `_install` before compiling. Today the skip fires because `links/<id>.cmake` exists, and that file is written at materialize, before install. Buffer at `-j32`, where Logger and System both need String/Base, starts `<consumer>_build` in parallel with the winner's `_build`/`_install`. The nested compile reads `BUILDMASTER_INSTALL_DIR/include` before `error.hxx` / `platform.h` / `cstring.hxx` exist (`file not found`). `-j1` hides it. A skipped configure is not a published prefix. Link: the consumer still needs the winner's transitive libraries even though this process did not configure String or Base; that closure is the next item. Suite workaround, remove when the wait exists: Buffer `thirdparty/StormByte/CMakeLists.txt` hoists String (`add_subdirectory(string)`) and adds `buildmaster_depend(StormByte-Logger StormByte-String)` and `buildmaster_depend(StormByte-System StormByte-String)`. Those comments already say to drop the subdirectory and both depends after this fix. Buffer's three `buildmaster_link` calls are not part of that workaround.
- [x] **Transitive `buildmaster_link`.** Contract: `buildmaster_link(A B)` names only B. BuildMaster adds every library B already carries — B's own `buildmaster_link` dests and `INTERFACE_LINK_LIBRARIES` — walked to the end, with a seen-set so a cycle is not followed. The caller does not re-list grandparents. Parent optstr `LINK=` does not rewrite the nested link line and is not a substitute. One hop already works in a single CMake process: `StormByte-String` PUBLIC-links `StormByte`, and `buildmaster_link(StormByte-String StormByte)` is enough there. Deeper than that, or the same edge read back after the component was installed or skipped, is not. `_bm_links_flatten_onto` is shaped like a closure (stack + seen) but the record is one hop. `_bm_links_write_one` stores the direct dests and copies dests-of-dests once; `_bm_links_write_all` makes exactly two passes, not a fixpoint. The links template only does `target_link_libraries(INTERFACE <dest>)` when `<dest>` is already a target, and ingest `include()`s each file exactly twice. `-l` / `Stem.lib` is emitted only for an id this process did not register, so the named hop is emitted and the next hop is either assumed to be in-graph (its INTERFACE was never filled) or absent from the file. Absence is concrete: a nested Logger that skips String never runs String's `buildmaster_link(StormByte-String StormByte)` or `add_subdirectory(base)`. The parent then rewrites `links/StormByte-String.cmake` from the edges it knows, which do not include String → Base, and the child file that listed Base is gone. Flatten stops at `-lStormByte-String`. That single `-lStormByte-String` on `libStormByte-Logger` is intentional. The missing `-lStormByte` is the bug. Without the suite workaround the objects still compile (headers were already there; this is not the race) and the link fails on Darwin's two-level namespace and on Windows: `ThreadLock`, `CString`, `WCString`, `Exception`, `Base64Encode`, plus the vtable and typeinfo for `StormByte::Exception`. Linux links because a DSO may leave those symbols undefined; runtime finds them through String's `DT_NEEDED` and `RPATH`. Temporary workaround, delete when the closure is deep: Logger `thirdparty/StormByte/CMakeLists.txt` names Base by hand with `buildmaster_link(StormByte-Logger StormByte StormByte-String)`. With that quirk Buffer's final link warns `ld: ignoring duplicate libraries: '-lStormByte'`. Buffer's own CMakeLists does not name `StormByte`. The duplicate is the workaround leaking onto a path that already forwarded `PUBLIC StormByte` (String built in Buffer's graph). End state is one `-lStormByte`, inserted by the closure, and no explicit grandparent. Regression: `base` ← PUBLIC `mid` ← `leaf` calling a Base-only symbol; `leaf` links on Darwin and Windows without naming `base`. A longer chain must resolve too, not only the second hop. If `mid` was skipped because an earlier component already configured it, `base` is still on `leaf`'s link line.
