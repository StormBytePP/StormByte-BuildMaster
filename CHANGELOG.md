...
- **Options:**
  ...
  - `WHOLE` flag (`WHOLE` ≡ `WHOLE=ON`) puts all produced **static** archives of the component on its INTERFACE inside one closed whole-archive region (ELF `--whole-archive` / `--no-whole-archive`, Apple `-force_load` per archive, MSVC `/WHOLEARCHIVE:`). Multiple WHOLE components emit **linear** closed regions (not nested). Other libraries linked alongside stay outside the region. On `shared` / `headers`: WARNING and ignored.
