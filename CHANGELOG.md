...
- [ ] **Transitive `buildmaster_link` across a nested component.** `buildmaster_link(A B)` must pull every library `B` already PUBLIC-links (its own `buildmaster_link` dests and `INTERFACE_LINK_LIBRARIES`). In one CMake tree that already works (`StormByte-String` PUBLIC-links `StormByte`). A nested child (Buffer hosts Logger as `buildmaster_component` + `WITH_STORMBYTE=BUNDLED`) only sees the installed mid artefact: the link line is `-lStormByte-String` / `StormByte-String.lib` with no Base. Darwin two-level namespace and Windows DLL link then fail (`ThreadLock`, `CString`, `Exception`, `Base64Encode`). Linux often still loads because ELF can leave those symbols undefined. Parent optstr `LINK=StormByte` does not rewrite the nested `clang++`/`lld-link` line. Consumers should not have to re-list grandparents. Regression: `base` ← PUBLIC `mid` ← `leaf` that calls a Base-only symbol; `leaf` must link on Darwin and Windows without naming `base`.
