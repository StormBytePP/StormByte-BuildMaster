...
- [x] **Idempotent stage stamps.** A later parent build that only changed the final project must not re-enter a leaf whose sources, patches and build inputs are unchanged. The tree half is a CMake digest of the srcdir after `GIT={PATCH}` (not `git rev-parse`: the tree may be a copied folder). The other half matches the install-cache inputs and is written to `bm-stamp.extra`. `bm-stamp.key` is written only after install succeeds. A hit skips configure, build and install. A miss runs them. `NOINSTALL` stamps the builddir artefacts. The cache itself is not implemented.
- [x] **Re-apply `GIT={PATCH}` before any rebuild.** A clean worktree is the invariant, so the next `make` does not see the patch. The stage applies it, hashes, and on a hit resets again without calling nested cmake/meson. On a miss the nested tool sees the patched tree, then reset runs when that git root has no live holder left. Canary: a `cmake_minimum_required` the running CMake rejects; the patch makes it legal; the second build must not parse the upstream file. Do not leave the worktree patched. Do not turn off regenerate-during-build.
