...
foreach(_c IN LISTS _cfg_cases)
	...
	if(_rc EQUAL 0)
		...
		elseif(_c STREQUAL "exe-pc")
			string(JOIN "\n" _blob "${_out}" "${_err}")
			string(FIND "${_blob}" "cannot be used with executable mode" _hit)
			if(_hit LESS 0)
				message(STATUS "negative/${_c} configure failed but not with exe+PC text:\n${_blob}")
				math(EXPR _failed "${_failed} + 1")
			else()
				message(STATUS "[BuildMaster/Core     ]: negative/${_c} configure-failed as required")
			endif()
		elseif(_c STREQUAL "exe-repack")
			string(JOIN "\n" _blob "${_out}" "${_err}")
			string(FIND "${_blob}" "REPACK is not valid in executable mode" _hit)
			if(_hit LESS 0)
				message(STATUS "negative/${_c} configure failed but not with exe+REPACK text:\n${_blob}")
				math(EXPR _failed "${_failed} + 1")
			else()
				message(STATUS "[BuildMaster/Core     ]: negative/${_c} configure-failed as required")
			endif()
		elseif(_c STREQUAL "exe-empty-produced")
			string(JOIN "\n" _blob "${_out}" "${_err}")
